...
int main(){
	...
	/*
	 ...
	 3 4 5 6
	*/
	treePointer temp_root = copy(root);//複製一棵樹 
	if(equal(root, temp_root)){
		printf("root = temp_root\n");
	}
	else{
		printf("root != temp_root\n");
	}
	...
	printf("level_order: ");
	level_order(root);//0123456
	printf("\ninorder: ");
	inorder(root);//3140526
	printf("\niteration_inorder: ");
	iteration_inorder(root);
	printf("\npreorder: ");
	preorder(root);//0134256
	printf("\npostorder: ");
	postorder(root);//3415620
	printf("\n");
} 
